...
contract OptionsContract is Ownable, ERC20 {
    ...
    */
    ...
    function issueOTokens(
        ...
        require(
            isSafe(getCollateral(vaultIndex), newOTokensBalance),
            "unsafe to mint"
        );
    ...
    function removeCollateral(uint256 vaultIndex, uint256 amtToRemove) public {
        ...
        require(
            amtToRemove <= getCollateral(vaultIndex),
        ...
        require(
            isSafe(newCollateralBalance, getOTokensIssued(vaultIndex)),
            "Vault is unsafe"
        );
    ...
    function claimCollateral(uint256 vaultIndex) public {
        ...
        uint256 collateralToTransfer = getCollateral(vaultIndex).div(10).mul(
            10
        );
        uint256 underlyingToTransfer = getCollateral(vaultIndex)
            .mul(totalUnderlying)
            .div(totalCollateral);
    ...
    function liquidate(uint256 vaultIndex, uint256 oTokensToLiquidate) public {
        ...
        uint256 maxCollateralLiquidatable = getCollateral(vaultIndex).mul(
    ...
    */
    function getOTokensIssued(uint256 vaultIndex)
        internal
        view
        returns (uint256)
    {
     ...
     */
    function _addCollateral(uint256 vaultIndex, uint256 amt)
        private
        returns (uint256)
    {
        require(!hasExpired(), "Options contract expired");
